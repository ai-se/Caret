...
SB<-function(datasets){
  ...
  Grid <-  expand.grid(  .trials = c(1:10),
                         .winnow = FALSE,
                         .model = "tree")
  ...
  Fit2 <- train(trainX, trainY,
                   ...
                   tuneGrid = Grid,
                   metric = "ROC")
  prob_predicted <-predict(Fit2, newdata = test_data, type = "prob")
  tuned_roc <-roc(predictor = prob_predicted$Y, response = test_data$bug, levels = rev(levels(test_data$bug)))
  result <-auc(tuned_roc)
...
#   train_Y <-train_data$bug
  ...
  Default_out <-roc(predictor = Default_predicted$Y, response = test_data$bug, levels = rev(levels(test_data$bug)))
